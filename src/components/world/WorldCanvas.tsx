...
import { computeBuoyancyVolumes, computeAmbientSoundZones, findDayNightComponent, computeSeats, computeTeleporters, computeLadders, computeDoors, computeDialogues, type BuoyancyVolume, type AmbientSoundZone, type ComponentInstance, type SeatSpot, type TeleporterSpot, type LadderSpot, type DoorSpot, type DialogueSpot } from '@/lib/world/components';
  ...
  useFrame((rtState, dt) => {
    ...
    if (body.current) {
      ...
      if (postFXZonesRef && onPostFXZone) {
        const zs = postFXZonesRef.current;
        let zi = -1;
        for (let i = 0; i < zs.length; i++) {
          const z = zs[i];
          if (Math.abs(posT.x - z.cx) <= z.hx && Math.abs(posT.y - z.cy) <= z.hy && Math.abs(posT.z - z.cz) <= z.hz) { zi = i; break; }
        }
        if (zi !== lastPostFXZone.current) { lastPostFXZone.current = zi; onPostFXZone(zi); }
      }
...
export default function WorldCanvas({ character, playerId, players, posesRef, chatBubbles, onMove, customObjects, sceneSettings, graphics = DEFAULT_SETTINGS, chatInputActive = false, emoteSlot, emoteOneShotOverride, sendScriptEvent, scriptEventRef, sendObjectStates, objectStatesRef, hostId, sendObjClaim, sendObjRelease, objectOwnerRef, sendObjSpawn, sendObjDestroy, objSpawnRef, objDestroyRef, sendSceneRegister, portalApiRef, onPortalEnter, cameraMode: cameraModeProp, onCameraModeChange, firstPersonFov = 75, worldId, socketRef, placementGhost }: WorldCanvasProps) {
  ...
  useEffect(() => { postFXZonesRef.current = postFXZones; }, [postFXZones]);
  ...
  useEffect(() => {
    ...
    scriptEventRef.current = (objectId, event, data, fromId) => {
      ...
      // 런타임 깎기 동기화 — 다른 클라가 world.carve 한 절삭을 본인도 적용 (보낸 클라는 이미 로컬 반영)
      if (event === '__carve__') {
  ...
  const carveObject = useCallback((id: string, opts: {
    shape?: 'box' | 'sphere' | 'cylinder'; x?: number; y?: number; z?: number; size?: number; world?: boolean;
  }): void => {
    if (!id) return;
    const shape = (opts.shape === 'box' || opts.shape === 'cylinder') ? opts.shape : 'sphere';
    ...
    if (opts.world) {
      // 월드 좌표 → 오브젝트 로컬 단위 큐브 좌표. 오브젝트의 기준 transform 사용(정적 깎기 대상).
    ...
    <>
      ...
      >
        ...
        <Suspense fallback={null}>
          ...
          {dialogues.length > 0 && (
            <DialogueController dialogues={dialogues} localPoseRef={localPoseRef} />
          )}
          ...
          <Physics gravity={[0, gravityY, 0]} interpolate={false}>
            {customObjects !== undefined ? (
              ...
              <>{(() => {
                ...
                let list = Object.keys(videoUrlOverrides).length === 0 ? base
                  : base.map(o => videoUrlOverrides[o.id] !== undefined ? { ...o, videoUrl: videoUrlOverrides[o.id] } : o);
                // 런타임 깎기(world.carve) 오버라이드 — csgCuts 병합. 깎인 오브젝트만 새 ref(그 큐브만 재렌더)
