...
function buildMat(cfg: any, onTex?: () => void): THREE.MeshStandardMaterial | null {
  ...
  if (cfg.textureAlbedo)  { mat.map = loadTex(cfg.textureAlbedo, THREE.SRGBColorSpace, tx, ty, trig); mat.alphaTest = 0.5; } // 알파 컷아웃
    ...
    import('@/lib/world/modelLoader').then(({ loadStaticModelCached }) =>
      loadStaticModelCached(obj.assetUrl!).then(model => {
        ...
        model.traverse(c => {
          ...
          mesh.userData.__windLeafSlots = slots;     // 슬롯별 잎 여부(줄기=all false 도 보관 → 굽힘은 받음, 펄럭임은 X)
          if (slots.some(Boolean)) anyLeaf = true;
          windMeshes.push(mesh);
        });
        // 오브젝트에 잎이 하나라도 있으면 = 식물 → 모든 메시(줄기 포함)가 바람 받음(굽힘). 잎 슬롯만 펄럭임.
  ...
  useEffect(() => {
    ...
    if (windActive && foliageRef.current && model) {
      ...
      model.traverse(c => {
        ...
        if (!mm.isMesh || !mm.material || !mm.userData?.__windFoliage) return;
        const slots = mm.userData?.__windLeafSlots as boolean[] | undefined;
        if (Array.isArray(mm.material)) mm.material.forEach((m, i) => patchWindMaterial(m, by, !!(slots && slots[i])));
        else patchWindMaterial(mm.material, by, !!(slots && slots[0]));
      });
    }
...
function SimScene({ objects, transforms, myAssets, player, gameApi, gameStore }: {
  ...
  const simTeleporters: TeleporterSpot[] = useMemo(() => {
    const merged = objects.map(o => { const t = transforms[o.id]; return t ? { ...o, position: t.pos, scale: t.scl } : o; });
    return computeTeleporters(merged);
  }, [objects, transforms]);
  const simLadders: LadderSpot[] = useMemo(() => {
    const merged = objects.map(o => { const t = transforms[o.id]; return t ? { ...o, position: t.pos, scale: t.scl } : o; });
  ...
  const simDialogues: DialogueSpot[] = useMemo(() => {
  ...
  const simJumpPads: JumpPadSpot[] = useMemo(() => {
  ...
  const simRaceStarts: RaceStartSpot[] = useMemo(() => {
    const merged = objects.map(o => { const t = transforms[o.id]; return t ? { ...o, position: t.pos, scale: t.scl } : o; });
    return computeRaceStarts(merged);
  }, [objects, transforms]);
  const simRaceFinishes: RaceFinishSpot[] = useMemo(() => {
    const merged = objects.map(o => { const t = transforms[o.id]; return t ? { ...o, position: t.pos, scale: t.scl } : o; });
    return computeRaceFinishes(merged);
  }, [objects, transforms]);
  ...
  return (
    <>
      ...
      {player && simTeleporters.length > 0 && (
        <TeleporterController teleporters={simTeleporters} localPoseRef={simPlayerPoseRef} playerCtlRef={playerCtlRef} />
      )}
      ...
      {player && (simRaceStarts.length > 0 || simRaceFinishes.length > 0) && (
        <RaceController starts={simRaceStarts} finishes={simRaceFinishes} localPoseRef={simPlayerPoseRef} />
      )}
                    ...
                    return (
                      <group key={'vr-' + obj.id} position={pos}>
                        <VideoRemotePanel
                          ...
                          crosshairAim={!!simCharacter && simCamView === 'follow'}
