'use client';
/**
 * 프로필 꾸미기 폼 — 페이지(/account/customize)와 모달 양쪽에서 재사용.
 *
 * 자체 상태로 load·save 처리. 저장 성공 시 onSaved 콜백 (optional, 모달 닫기 등).
 */
...
interface Props {
  /** 저장 성공 시 호출. 모달에선 닫기 트리거로 사용. */
  onSaved?: () => void;
  /** "뒤로" 또는 "닫기" 버튼 라벨·동작 커스터마이즈. 미제공 시 버튼 안 보임. */
  onClose?: () => void;
  closeLabel?: string;
}

export default function ProfileCustomizeForm({ onSaved, onClose, closeLabel }: Props) {
  ...
  if (needsLogin) {
    return <p className="text-slate-400">{t('loginRequired')}</p>;
  }
