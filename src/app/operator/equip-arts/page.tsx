...
export default function OperatorEquipArtsPage() {
  ...
  const loadStatus = useCallback(async () => {
    ...
    setLoadError(null);
    try {
      const res = await api.operatorEquipArtStatus(t);
      setTotal(res.total);
      setItems(res.items);
    } catch (err) {
  ...
  async function generateOne(noun: string) {
    ...
    try {
      await api.craftEquipArtGenerateOne(t, noun);
      ...
      setGenErrors((prev) => ({ ...prev, [noun]: err instanceof Error ? err.message : "생성 실패" }));
    } finally {
    ...
    <div className="mx-auto w-full max-w-4xl px-4 py-10">
      ...
      <div className="mb-6 flex items-end justify-between">
        <div>
          ...
          <h1 className="mt-0.5 text-2xl font-bold">대장간 장비 이미지 관리</h1>
        </div>
      ...
      {filteredMissingCount > 0 && !loading && (
        <div className="mb-4 flex items-center gap-3">
          ...
          >
            {anyGenerating ? `생성 중… (${generating.size}건)` : `미생성 ${filteredMissingCount}건 전체 생성`}
          </button>
          <p className="text-xs text-zinc-500">순차적으로 생성합니다.</p>
        </div>
      )}
